...
/*Lo que pense fue que dado que ya esta un constrint para financiamiento que usa la
  clave PpalFuente puse todo para estar junto, pues es clave primaria, funciono
  pero no tiene sentido poner eso, asi que no se que hacer*/

ALTER TABLE "dycicle".Recibe ADD
        CONSTRAINT FK_Recibe_Financiamiento FOREIGN KEY (NombreUsuario,PpalFuente)
